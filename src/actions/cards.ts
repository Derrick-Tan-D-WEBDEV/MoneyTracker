...
export async function searchCatalog(query: string, opts?: { setCode?: string; limit?: number }): Promise<CatalogCard[]> {
  ...
  for (const c of remote) {
    ...
    const data = {
      ...
      inkCost: typeof c.ink_cost === "number" ? c.ink_cost : null,
      cardType: c.type?.join(", ") ?? null,
      ink: c.ink ?? null,
        ...
        if (match) {
          ...
          if (lookup) {
            const data = {
              ...
              inkCost: typeof lookup.ink_cost === "number" ? lookup.ink_cost : null,
              cardType: lookup.type?.join(", ") ?? null,
              ink: lookup.ink ?? null,
