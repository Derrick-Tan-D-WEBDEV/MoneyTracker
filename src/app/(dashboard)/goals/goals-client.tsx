...
export function GoalsClient() {
  ...
  // Helper: format in goal's account currency or user default
  const fmtGoal = (goal: Goal, amount: number) => {
    const currency = goal.account?.currency || userCurrency;
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              {savingsAccounts.length > 0 && (
                <div className="space-y-2">
                  ...
                  <Select value={formAccountId || "none"} onValueChange={(v) => setFormAccountId(v === "none" ? null : v)}>
                    <SelectTrigger>
                      <SelectValue>
                        {(value: string) => {
